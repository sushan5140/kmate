...
-- =========================================================================
-- YOUTUBE REPLY RECOVERY ATTEMPTS
-- =========================================================================
-- A recovery is a separate child attempt. It never reopens or overwrites the
-- original queue row, whose REMOVED state, final_draft and posted_reply_id
-- remain historical truth. queue_id is nullable because some old bot history
-- predates youtube_reply_queue; youtube_comment_id remains the parent identity.
create table if not exists public.youtube_reply_recovery_attempts (
  id uuid primary key default gen_random_uuid(),
...
-- One active/live workflow per parent. Only terminal SKIP/REMOVED releases it.
create unique index if not exists youtube_reply_recovery_attempts_active_parent_key
...
create unique index if not exists youtube_reply_recovery_attempts_posted_reply_id_key
  on public.youtube_reply_recovery_attempts (posted_reply_id)
  where posted_reply_id is not null;

create index if not exists youtube_reply_recovery_attempts_queue_idx
  on public.youtube_reply_recovery_attempts (queue_id)
  where queue_id is not null;

create index if not exists youtube_reply_recovery_attempts_review_idx
