...
interface MistakeRow {
  ...
  mistake_upvotes: { user_id: string }[];
}
...
export default async function MistakesPage() {
  ...
  const { data } = await supabase
    ...
    .select(
      "id, title, description, document_type, reason_category, upvotes_count, confidence, source_url, mistake_upvotes ( user_id )"
    )
  ...
  const entries: MistakeEntryData[] = ((data ?? []) as unknown as MistakeRow[]).map((e) => ({
    ...
    upvotedByMe: e.mistake_upvotes.some((u) => u.user_id === user.id),
