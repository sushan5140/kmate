...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Independently re-checks admin authorization server-side -- never trust
  // that the page-level gate alone protected this route.
  ...
  const rateLimit = checkRateLimit(`moderate-questions:${user.id}`, 20, 5 * 60 * 1000);
